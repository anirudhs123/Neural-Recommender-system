def Turicreate_collab_train(path,israted=False):
  ...
  models_w_dummy = [cos]
  names_w_dummy = ['Cosine Similarity on Purchase ']
  eval_dummy = tc.recommender.util.compare_models(test_data, models_w_dummy, model_names=names_w_dummy)

  cosine_model = tc.item_similarity_recommender.create(tc.SFrame(trains), 
                                            user_id=user_id, 
                                            item_id=item_id, 
                                            target='binary', similarity_type='cosine')
  recom_cos = cosine_model.recommend(users=users_to_recommend, k=5)
  recom_cos.print_rows(n_display)
  df_rec_cos = recom_cos.to_dataframe()

  

  return(test,recom_cos,df_rec_cos)

def predictions_model_cos(test,df_rec_cos):
    ...
    pred_user_dict_cos={}
    for i in list(df_rec_cos['User_Id'].unique()):
    	pred_user_dict_cos[i]=[]

    for i in range(len(df_rec_cos)):
    	if(ratings_matrix[df_rec_cos['User_Id'][i]][df_rec_cos['Item_Id'][i]]!=-1):
    		pred_user_dict_cos[df_rec_cos['User_Id'][i]].append(df_rec_cos['Item_Id'][i])

    f=0
    for i in usr_dict.keys():
    	for j in pred_user_dict_cos[i]:
    		if(j in usr_dict[i]):
    			f=f+1
    			break
    print('Accuracy of Cosine_Similarity Recommender : {}'.format(f/len(usr_dict)))
    		

    return(predict_user_dict_cos)
